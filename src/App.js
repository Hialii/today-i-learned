...
function NewFactForm({ setFacts, setShowForm }) {
  ...
  return (    
     <form className="fact-form" onSubmit={handleSubmit}>
        <input 
        value={text} type="text" disabled={isUploanding}
         placeholder="Share a fact with the world..."  
         onChange={(e) => setText(e.target.value)}/>
        
        <span>{200 - textLength}</span>
        <input disabled={isUploanding} value={source} type="text" 
        placeholder="Trustworthy source..."
        onChange={(e) => setSource(e.target.value)} />
        <select disabled={isUploanding} value={category} onChange={((e) => setCategory(e.target.value))}>
          <option value="">Choose category</option>
          {colorArr.map((cat) => <option key={cat.name} value={cat.name}>{cat.name.toUpperCase()}</option>)}

        </select>
        <button className="btn btn-large" disabled={isUploanding}>Post</button>
      </form>
 )
}
...
function CategoryFilter({ setCurrentCategory }) {
  ...
  <aside>
    <ul>
      ...
      {colorArr.map((cat) => ( <li key={cat.name} className="category">
      <button className="btn btn-category" style={{backgroundColor: cat.color}} onClick={() => setCurrentCategory(cat.name)} >{cat.name}</button></li>)) }  
    </ul>
 </aside>
  );
}
...
function FactList({ facts, setFacts }) {
  ...
  if(facts.length === 0) 
    return <p className='message'>No facts for this category yet. Creat the firt one!</p>
  return(
     <section>
       <ul className="facts-list">
          {facts.map((fact) => <Fact  key={fact.id} setFacts={setFacts}  fact ={fact}/>)}
        </ul>
...
function Fact({ fact, setFacts}) {
  ...
  return <li className="fact">
        ...
        <span className="tag" style={{backgroundColor: colorArr.find((cat) => cat.name === fact.category).color}}>{fact.category}</span>
